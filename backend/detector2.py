...
def run_detection(video_path: str, line_ratio: float = 0.65, conf: float = 0.40, flip_line: bool = False):
    """
    ...
    Only inward crossings (people entering) are tracked for violations.
    People leaving the gym (outward crossings) are completely ignored.
    Use flip_line=True if your camera has entrants moving upward in the frame.
    """
    ...
    # ── Counting line ──────────────────────────────────────────────────────────
    # Supervision defines "in" as crossing from the start-point side to the
    # end-point side. For a horizontal line:
    #   start=left, end=right  →  top-to-bottom movement = IN  (default)
    #   start=right, end=left  →  bottom-to-top movement = IN  (--flip-line)
    #
    # Run once, watch the dashboard in/out counts, then use --flip-line
    # if entries are registering as "out" instead of "in".
    ...
    while cap.isOpened():
        ...
        # crossed_in  — boolean array, True where a detection crossed inward
        # crossed_out — boolean array, True where a detection crossed outward
        # We ONLY use crossed_in. Two members leaving together is not a violation.
        ...
        # Draw counting line with live in/out counts
        ...
        if is_violation:
            ...
            cv2.putText(
                annotated,
                f"TAILGATE DETECTED  {len(recent_entrants)} PEOPLE / 1 SWIPE",
                (20, height - 30),
        ...
        cv2.rectangle(annotated, (0, 0), (320, 28), (0, 0, 0), -1)
        cv2.putText(
            annotated,
        ...
        cv2.rectangle(annotated, (0, height - 28), (220, height), (0, 0, 0), -1)
        cv2.putText(
            annotated,
            f"Entries (in): {line_zone.in_count}",
            (8, height - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
            (0, 80, 255) if len(recent_entrants) > 1 else (80, 220, 80), 1
        )
        ...
        with state_lock:
            ...
            state["total_entries"]    = line_zone.in_count  # inward crossings only
            if frame_b64:
...
@app.post("/start")
def start_detection(video_path: str, line_ratio: float = 0.65, conf: float = 0.40, flip_line: bool = False):
    if state["running"]:
    ...
    t = threading.Thread(
        ...
        args=(video_path, line_ratio, conf, flip_line),
        daemon=True
    )
    t.start()
